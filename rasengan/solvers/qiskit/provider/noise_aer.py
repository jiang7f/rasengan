...
class NoiseAerProvider(Provider):
    ...
    def get_counts(self, qc: QuantumCircuit, shots: int) -> Dict:
        ...
        self.sim_noise = AerSimulator(noise_model=self.noise_model, shots=shots,method="statevector", seed_simulator=0x81bb97f)
    ...
    def transpile(self, qc: QuantumCircuit) -> QuantumCircuit:
        self.sim_noise = AerSimulator(noise_model=self.noise_model, method="statevector", seed_simulator=0x81bb97f)
        # Transpile circuit for noisy basis gates
        circ_tnoise = transpile(qc, self.sim_noise)
        return circ_tnoise
