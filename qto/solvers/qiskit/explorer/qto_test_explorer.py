...
class QtoTestExplorer(Explorer):
    def __init__(
        ...
        *,
        ...
        if feasible_state is None:
            ...
            print("n: ", len(self.feasible_state))
        else:
            self.feasible_state = feasible_state
            print("n: ", len(self.feasible_state[0]))
    ...
    def explore(self):
        ...
        import math
        print("min_prob: ", min(dict_prob.values()), math.log2(min(dict_prob.values())))
        print("max_prob: ", max(dict_prob.values()), math.log2(max(dict_prob.values())))
        print("max_split: ", max_split)
        print("longest_train: ", longest_train)
        return dict_state, max_order, counter, max_split, longest_train
    ...
    def print_generation_relationship(self):
        ...
        # 文字版
        print("State and their indices:")
        for state, (index, preorder_counter, order) in dict_state.items():
            print(f"State: {state}, Index: {index}, Preorder Counter: {preorder_counter}, Order: {order}")

        print("\nGeneration relationships:")
        print("Start:")
        for state, (index, preorder_counter, order) in dict_state.items():
            if preorder_counter is None:
                print(f"Index {index} generated by {preorder_counter}")
